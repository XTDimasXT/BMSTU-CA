...
def Gauss(matrixSlau):
    ...
    # находим неизвестные обратным ходом метода Гаусса
...
def funcm2_or_3(xValues, equation):
    yValues = []
    for n in xValues:
        x = n
...
def drawGraph(xValues, func, equation2):
    plt.figure("График(и) функции, полученный аппроксимации наименьших квадратов")
    plt.ylabel("Y")
    plt.xlabel("X")
    
    #yValues = func(xValues, equation1)
    #plt.plot(xValues, yValues, 'g', label="y = f(x) (m=2)")
    
    yValues = func(xValues, equation2)
    plt.plot(xValues, yValues, 'r', label="y = f(x) (m=3)")
...
def equation_solution():
    ...
    #equation_m2 = "(1 - x) + {:.3f}*(x * (1-x)) + {:.3f}*(x**2 * (1-x))".format(a[0], a[1])
    equation_m3 = "(1 - x) + {:.3f}*(x * (1-x)) + {:.3f}*(x**2 * (1-x)) + {:.3f}*(x**3 * (1-x))".format(a[0], a[1], a[2])
    
    print("Ответ: y(x) = (1 - x) + {:.3f}*(x(1-x)) + {:.3f}*(x^2(1-x)) + {:.3f}*(x^3(1-x))".format(a[0], a[1], a[2]))