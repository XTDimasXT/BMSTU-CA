...
def changeWeigth(pointTable):
    answer = 0
    while answer == 0:
        index = input("Введите номер точки (для окончания ввода введите enter): ")
        if index == '':
            break
        index = int(index)
        if index <= 0 or index > len(pointTable):
            print("Такого номера точки нет! Изменение завершено")
            answer = 1

        else:
            w = int(input("Введите вес точки: "))
            if w < 1:
                print("Некорректный вес! Изменение завершено")
                answer = 1
            else:
...
def inputApproximateDegree():
    ...
    while n < 0:
        ...
        if n < 0:
            print(" Предупреждение! Степень аппроксимации не может быть меньше 0!")
